...
const DogGrooming400OffPage: PageComponent = async props => {
  ...
  const [ dgPrice, dePrice ] = await Promise.all([
    fetchPrice([ 'dg' ], countryCode, provinceCode, { promoCode: 'DG400', school: 'QC Pet Studies' }, undefined, process.env.FIREWALL_BYPASS_SECRET),
    fetchPrice([ 'de' ], countryCode, provinceCode, { promoCode: 'DG400', school: 'QC Pet Studies' }, undefined, process.env.FIREWALL_BYPASS_SECRET),
  ]);
  ...
  return (
    <>
      ...
      <DogGroomingBase countryCode={countryCode} provinceCode={provinceCode} dgPrice={dgPrice.value} dePrice={dePrice.value} enrollPath="/grooming-400-off" courseCode={courseCode} />
    </>
  );
};
