...
export const metadata: Metadata = {
  ...
  alternates: { canonical: '/certification-courses/dog-training/500-off' },
};

const courseCodes: CourseCode[] = [ 'dt' ];

const DogTraining500OffPage: PageComponent = async props => {
  const { countryCode, provinceCode } = await getServerData(props.searchParams);

  const price = await fetchPrice(courseCodes, countryCode, provinceCode, { promoCode: 'DT500', school: 'QC Pet Studies' }, undefined, process.env.FIREWALL_BYPASS_SECRET);
  if (!price.success) {
    return null;
  }
  ...
  return (
    <>
      ...
      <DogTrainingBase price={price.value} enrollPath="/training-500-off" courseCodes={courseCodes} countryCode={countryCode} provinceCode={provinceCode} dtPrice={dtPrice.value} dePrice={dePrice.value} />;
    </>
  );
};

export default DogTraining500OffPage;
