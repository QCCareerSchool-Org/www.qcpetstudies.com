...
import { BrochureForm, CourseCode } from '../components/BrochureForm';
...
const formActions: Record<CourseCode, string> = {
  dg: 'https://go.qcpetstudies.com/l/947642/2021-12-05/6h9rv',
  dt: 'https://go.qcpetstudies.com/l/947642/2021-12-05/6h9rx',
};
...
const DogGroomingCatalogPage: NextPageWithLayout<Props> = ({ firstName, lastName, emailAddress, emailOptIn, telephoneNumber, smsOptIn, errors, testGroup, gclid, msclkid, marketing }) => {
  ...
  const [ formAction, setFormAction ] = useState(formActions[defaultCourse]);

  const handleBrochureBottomLinkClick: MouseEventHandler = () => {
    gaEvent('click', { id: 'brochureBottomLink' });
  };

  const handleCourseChange = (courseCode: CourseCode): void => {
    setCourses([ courseCode ]);
    setFormAction(formActions[courseCode]);
  };
